...
const AsnDetail = () => {
  ...
  const getCompletionPercent = () => {
    ...
    return Math.round((asnData.total_received_units / asnData.total_expected_units) * 100);
  };
  ...
  // Handle cancel ASN - UPDATED ENDPOINT
  const handleCancelASN = async () => {
    if (window.confirm("Are you sure you want to cancel this ASN? This action cannot be undone.")) {
      try {
        // Try different possible cancel endpoints
        let cancelResponse;
        try {
          // First try: /asns/{id}/cancel
          cancelResponse = await http.post(`/asns/${id}/cancel`);
        } catch (endpoint1Error) {
          console.log("Trying alternative cancel endpoint...");
          // Second try: /asns/{id} with DELETE method
      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            {asnData.status !== "CLOSED" && asnData.status !== "CANCELLED" && asnData.status !== "PUTAWAY_COMPLETED" && (
              <button 
            ...
            {asnData.status === "IN_RECEIVING" && (
              <button 
                onClick={() => navigate(`/ASNreceive/${asnData.asn_no}`, { state: { id: asnData.id } })}
      ...
      <div className="mt-6 space-y-6">
          ...
          <div className="lg:col-span-7 h-full">
            <KeyValueCard
              ...
              items={[
                ...
                { label: "Carrier / Vehicle", value: `${asnData.transporter_name || "N/A"} / ${asnData.vehicle_no || "N/A"}` },
                ...
                { label: "Special Handling", value: asnData.special_handling || "None" },
                { label: "Notes", value: asnData.notes || "None" },
                { label: "Created By", value: asnData.creator?.username || "N/A" },
                { label: "Created At", value: new Date(asnData.created_at).toLocaleString() },
              ]}
            />
          </div>
        ...
        {asnData.lines && asnData.lines.length > 0 && (
          <div className="rounded-lg border border-gray-200 bg-white p-4">
            <div className="text-sm font-semibold text-gray-900 mb-4">ASN Lines</div>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead>
                  <tr className="bg-gray-50">
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">SKU Code</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">SKU Name</th>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Damaged Qty</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Shortage Qty</th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {asnData.lines.map((line) => (
                    <tr key={line.id}>
                      <td className="px-4 py-3 text-sm text-gray-900">{line.sku?.sku_code || "N/A"}</td>
                      <td className="px-4 py-3 text-sm text-gray-900">{line.sku?.sku_name || "N/A"}</td>
                      <td className="px-4 py-3 text-sm text-gray-500">{line.uom || "N/A"}</td>
                      <td className="px-4 py-3 text-sm text-gray-900">{line.expected_qty}</td>
                      <td className="px-4 py-3 text-sm text-gray-900">{line.received_qty}</td>
                      <td className="px-4 py-3 text-sm text-gray-900">{line.damaged_qty}</td>
                      <td className="px-4 py-3 text-sm text-gray-900">{line.shortage_qty}</td>
                      ...
                      <td className="px-4 py-3 text-sm text-gray-500">{line.remarks || "-"}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr className="bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm font-semibold text-gray-900">
                      {asnData.lines.reduce((sum, line) => sum + (line.expected_qty || 0), 0)}
                    </td>
                    <td className="px-4 py-3 text-sm font-semibold text-gray-900">
                      {asnData.lines.reduce((sum, line) => sum + (line.received_qty || 0), 0)}
                    </td>
                    <td className="px-4 py-3 text-sm font-semibold text-gray-900">
                      {asnData.lines.reduce((sum, line) => sum + (line.damaged_qty || 0), 0)}
                    </td>
                    <td className="px-4 py-3 text-sm font-semibold text-gray-900">
                      {asnData.lines.reduce((sum, line) => sum + (line.shortage_qty || 0), 0)}
                    </td>
                    <td colSpan="2"></td>
                  </tr>
                </tfoot>
