...
import { Play, ChevronDown, ChevronRight, MapPin, Package, Clock, Hash } from "lucide-react";
...
const AllocationTab = () => {
  ...
  const rows = useMemo(() => {
    ...
    allocations.forEach(allocation => {
      ...
      groupedBySku[skuId].allocated += parseFloat(allocation.allocated_qty || 0);
    });
  ...
  const columns = useMemo(
    () => [
      ...
      { 
        key: "allocated", 
        title: "Allocated",
        render: (r) => <span className="font-medium text-green-600">{r.allocated}</span>
      },
      {
        key: "short",
        title: "Short",
        render: (r) => (
          <span className={parseFloat(r.short) > 0 ? "text-red-600 font-semibold" : "text-gray-600"}>
            {r.short}
          </span>
        ),
      },
  ...
  const renderExpandedDetails = (row) => {
    if (!expandedRows.has(row.id) || !row.allocations || row.allocations.length === 0) {
      return null;
    }
        ...
        <div className="px-4 py-3">
          ...
          <div className="space-y-3">
            {row.allocations.map((allocation) => {
              ...
              return (
                <div key={allocation.id} className="bg-white border rounded-lg p-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {/* Allocation Info */}
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        <Hash size={14} className="text-gray-400" />
                        <span className="text-xs text-gray-500">Allocation No</span>
                      </div>
                      <div className="font-medium text-sm">{allocation.allocation_no}</div>
                      <div className="text-xs text-gray-600">
                        <Pill text={allocation.status} tone={allocation.status === "CONSUMED" ? "green" : "blue"} />
                      </div>
                    </div>
                        ...
                        {allocation.consumed_at && (
                          <div className="text-xs">
                            ...
                            <span className="font-medium text-green-600">
                              {new Date(allocation.consumed_at).toLocaleDateString()}
                            </span>
                          </div>
                        )}
                      </div>
      ...
      <div className="space-y-4">
        <div className="rounded-lg border border-gray-200 bg-white p-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="text-sm text-gray-600">No allocations found for this order</div>
            <div className="flex items-center gap-2">
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-gray-600">
                Full Lines <span className="ml-1 font-semibold text-gray-900">{stats.full}</span>
              </span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-orange-500"></div>
              <span className="text-sm text-gray-600">
                Partial Lines <span className="ml-1 font-semibold text-orange-600">{stats.partial}</span>
              </span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-gray-300"></div>
              <span className="text-sm text-gray-600">
                Unallocated <span className="ml-1 font-semibold text-gray-900">{stats.unallocated}</span>
              </span>
            </div>
          </div>
