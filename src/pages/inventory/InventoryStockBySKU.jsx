...
const InventoryStockBySKU = () => {
  ...
  const fetchClients = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        const clientOptions = ["All", ...clientList.map(c => c.client_name)];
  ...
  const fetchInventoryData = async () => {
    try {
      ...
      for (const endpoint of endpoints) {
        try {
          ...
          if (f.stockStatus && f.stockStatus !== "All") {
            ...
            params.append('status', statusMap[f.stockStatus] || f.stockStatus.toUpperCase());
          }
      ...
      const summaryData = inventoryResponse.data?.summary || {
        total_on_hand: inventory.reduce((sum, item) => sum + (item.on_hand_qty || item.on_hand || 0), 0),
        total_available: inventory.reduce((sum, item) => sum + (item.available_qty || item.available || 0), 0),
        total_hold: inventory.reduce((sum, item) => sum + (item.hold_qty || item.hold || 0), 0),
        total_allocated: inventory.reduce((sum, item) => sum + (item.allocated_qty || item.allocated || 0), 0),
        total_damaged: inventory.reduce((sum, item) => sum + (item.damaged_qty || item.damaged || 0), 0),
        locations: new Set(inventory.map(item => item.location_id).filter(Boolean)).size
      };
      ...
      if (inventory.length > 0) {
        const zonesFromInventory = ["All", ...new Set(
          inventory
  ...
  const columns = [
      ...
      render: (r) => (
        <div className="flex items-center gap-3">
          ...
          <div className="leading-tight">
            ...
            <div className="text-xs text-gray-400">{r.category} • {r.uom}</div>
          </div>
        </div>
      ),
    },
    ...
    { 
      key: "available", 
      title: "Available",
      render: (r) => <span className="font-medium text-green-600">{r.available}</span>
    },
    { 
      key: "hold", 
      title: "Hold",
      render: (r) => <span className="font-medium text-orange-600">{r.hold}</span>
    },
    { 
      key: "allocated", 
      title: "Allocated",
      render: (r) => <span className="font-medium text-blue-600">{r.allocated}</span>
    },
    {
      key: "damaged",
      title: "Damaged",
      render: (r) => <span className="font-medium text-red-600">{r.damaged}</span>
    },
    {
      key: "locations",
      title: "Locations",
      render: (r) => (
        <div className="text-xs text-gray-600 max-w-[120px] truncate" title={r.locations}>
          {r.locations}
        </div>
      ),
    },
  ...
  const testApiManually = async () => {
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        toast.success(`${endpoint.name}: ${response.data.success ? 'Success' : 'Failed'}`);
      } catch (error) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-end gap-2">
        ...
        <button
          onClick={() => {
            console.log("Current state:", { f, warehouses, clients, zones, inventoryData, summary });
      ...
      {loading ? (
        ...
        <>
          <div className="text-sm text-gray-500 mb-2">
            ...
            {f.warehouse !== "All" && ` in warehouse ${warehouses.find(w => w.value === f.warehouse)?.label || f.warehouse}`}
          ...
          <div className="text-xs text-gray-400 text-center">
            Showing aggregated view by SKU. Click "View" to see location-level details.
          </div>
        </>
      )}
    </div>
